        ...
        do ipx = 0,npx-1
            ...
            if ( rank .eq. pidmap(ipz,ipy,ipx) ) then
                ...
                do i = 1,nlgrd
                    ...
                    tmp2 = 2.0*(h_ijm(i,:,1)+h_ijm(i,:,2)+h_ijm(i,:,3)) + &
                              h_ijm(i,:,4)+h_ijm(i,:,5)+h_ijm(i,:,6)
                    
                    ! Feng's understanding to the formula 18, Li 2005 paper ?
                    ...
                    do ifreq=1,nfreq
                        ...
                        ! Po Chen's updated beta kernel
                        tmpbp = -2*Vs(i)*Rho(i) * &
                                (tmp2(it_1:it_2)-2*tmp1(it_1:it_2)) * &
                                iDt(ifreq,it1:it2)
                        tmpbq = -2*Vs(i)*Rho(i) * &
                                (tmp2(it_1:it_2)-2*tmp1(it_1:it_2)) * &
                                rDt(ifreq,it1:it2)
